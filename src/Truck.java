        ...
        for (int i = nextFreeCell[0]; i < nextFreeCell[0] + item.getWidthX(); i++) {
            for (int j = nextFreeCell[1]; j < nextFreeCell[1] + item.getHeightY(); j++) {
                for (int k = nextFreeCell[2]; k < nextFreeCell[2] + item.getLengthZ(); k++) {
                    try {
                      //  System.out.println("trying to put item " + item.getWidthX() + " "
                     //           + item.getHeightY() + " " + item.getLengthZ());
                      //  System.out.println("at position " + i + " " + j + " " + k);
                      //  System.out.println("the area used for this will be in x " +
                      //          nextFreeCell[0] + " until " + (nextFreeCell[0] + item.getWidthX())
                        //        + " in y direction " + nextFreeCell[1] + " until " + (nextFreeCell[1] + item.getHeightY())
                          //      + " in z direction" + nextFreeCell[2] + " until " + (nextFreeCell[2] + item.getLengthZ()));
        ...
        for (int i = 0; i < widthX; i++){
            for (int j = 0; j < heightY; j++) {
                for (int k = 0; k < lengthZ; k++) {
                    ...
                    else if  (this.cargoSpace[i][j][k] < 4.99 && this.cargoSpace[i][j][k] > 4.00) {
